...
use crate::{FunctionContext, Parser, RegularTypeNameContext};
use ast::*;
use lexer::Token;
...
impl<'ast> Parser<'ast> {
    pub fn function_definition(&mut self) -> Option<ContractPartNode<'ast>> {
        ...
        loop {
            match self.lexer.token {
                Token::KeywordExternal => {
                    self.unique_flag(&mut visibility, FunctionVisibility::External)
                }
                Token::KeywordPublic => {
                    self.unique_flag(&mut visibility, FunctionVisibility::Public)
                }
                Token::KeywordInternal => {
                    self.unique_flag(&mut visibility, FunctionVisibility::Internal)
                }
                Token::KeywordPrivate => {
                    self.unique_flag(&mut visibility, FunctionVisibility::Private)
                }

                Token::KeywordPure => self.unique_flag(&mut mutability, StateMutability::Pure),
                Token::KeywordConstant => {
                    self.unique_flag(&mut mutability, StateMutability::Constant)
                }
                Token::KeywordView => self.unique_flag(&mut mutability, StateMutability::View),
                Token::KeywordPayable => {
                    self.unique_flag(&mut mutability, StateMutability::Payable)
                }
    ...
    fn modifier_invocation(&mut self) -> Option<Node<'ast, ModifierInvocation<'ast>>> {
        ...
        self.node_at(id.start, end, ModifierInvocation { id, arguments })
    }
    ...
    fn parameter(&mut self) -> Option<Node<'ast, Parameter<'ast>>> {
        ...
        self.node_at(type_name.start, end, Parameter { type_name, name })
    }
}
