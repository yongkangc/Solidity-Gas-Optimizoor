...
impl<'ast> Parser<'ast> {
    ...
    fn pragma_directive(&mut self) -> Option<SourceUnitNode<'ast>> {
        ...
        self.node_at(start, end, PragmaDirective { version })
    }
    ...
    fn import_directive_from(&mut self, start: u32) -> Option<SourceUnitNode<'ast>> {
        ...
        self.node_at(start, end, ImportDirective::Global { source, alias })
    }
    ...
    fn import_node(&mut self) -> Node<'ast, Import<'ast>> {
        ...
        self.node_at(symbol.start, end, Import { symbol, alias })
    }
