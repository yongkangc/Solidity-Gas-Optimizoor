...
type HandlerFn =
    for<'ast> fn(&mut Parser<'ast>, ExpressionNode<'ast>) -> Option<ExpressionNode<'ast>>;
...
const CALL: HandlerFn = |par, callee| {
    ...
    par.node_at(callee.start, end, CallExpression { callee, arguments })
};
...
const MEMBER: HandlerFn = |par, object| {
    ...
    par.node_at(
        object.start,
        member.end,
        MemberAccessExpression { object, member },
    )
};
...
const INDEX: HandlerFn = |par, array| {
    ...
    par.node_at(array.start, end, IndexAccessExpression { array, index })
};
...
const INC: HandlerFn = |par, operand| {
    ...
    par.node_at(
        operand.start,
        operator.end,
        PostfixExpression { operator, operand },
    )
};
...
impl<'ast> Parser<'ast> {
    ...
    pub fn nested_expression(
        &mut self,
        mut left: ExpressionNode<'ast>,
        precedence: Precedence,
    ) -> ExpressionNode<'ast> {
        while let Some(node) = NESTED_LUT[self.lexer.token as usize]
            .get(precedence)
            .and_then(|handler| handler(self, left))
        {
            left = node;
        }
                                        ...
                                        m.stmt_expr(
                                            ...
                                            IndexAccessExpression {
                                                ...
                                                index: m.node(
                                                    147,
                                                    148,
                                                    Primitive::IntegerNumber("1", NumberUnit::None),
                                                ),
                                            },
                                        ),
                                        m.stmt_expr(
